...
class Prof1():
    def __init__(self):
        ...
        self.response_size_mix_list_kb = [0.2, 6, 8, 9, 10, 25, 26, 35, 59, 347]
    ...
    def create_profile(self):
        ...
        http_response = lambda resp_size: self.http_response_template.format(int(resp_size),
                                                                             '*' * ( int(resp_size) - self.response_body_tags_bytes ))
        # client commands
        prog_c = ASTFProgram()
        for size in self.response_size_mix_list_kb:
            prog_c.send(self.http_req)
            prog_c.recv(len(http_response(size*1024)))
        # implicit  close
        ...
        for size in self.response_size_mix_list_kb:
            ...
            prog_s.send(http_response(size*1024))
